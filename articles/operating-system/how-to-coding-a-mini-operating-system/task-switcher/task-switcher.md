...
这在 Cortex-M3 中有一个叫做 PendSV 中断就是专门做这个事情的。
...
![C header export](./c-header-export.png)
...
![Init task function](./init-task-function.png)
...
![Task manager performance UI](./taskmgr-performance-ui.png)
...
![System Idle Process](./system-idle-process.png)

![System Idle Process in Windows 10](./system-idle-process-in-win10.png)
...
![Task idle function](./task-idle-function.png)
...
![Create task function](./create-task-function.png)
...
![CM3 double stack](./cm3-double-stack.png)
...
![Interrupt behavior](./interrupt-behavior.png)
...
![PendSV handler PROC](./pendsv-handler-proc.png)
...
![Switch current TCB function](./switch-current-tcb-function.png)
...
## 恢复上下文
恢复上下文这一个步骤才是真正做了任务切换，因为我们将新的任务的上下文中事先在栈段里面保存的 PC 值恢复到了 CPU 的 PC 寄存器中，因此这一步才是整个任务切换中至关重要的一步。
...
![Update register](./update-register.png)

![Interrupt return value](./interrupt-return-value.png)
