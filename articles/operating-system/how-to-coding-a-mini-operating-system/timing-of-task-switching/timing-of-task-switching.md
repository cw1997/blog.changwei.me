...
因为我们即使看遍了整个 ez-rtos 的源代码，也并没有发现有什么地方显式调用了 PendSV_Handler 这个函数。
...
![SysTick timer](./systick-timer.png)
...
![Define config sysclk](./define-config-sysclk.png)
...
![SysTick handler calls switch task](./systick-handler-call-switch-task.png)
...
OS 为了实现用户代码操作硬件，就事先将一些操作硬件的代码封装成系统调用函数，通过中断的方式请求系统调用，系统调用进入内核级再去操作具体的硬件。由于系统调用是 OS 的开发者编写的，是经过验证和测试的，不会有什么危险操作。因此既满足了用户需要操作硬件的需求，又没有让用户代码直接去操作硬件。
...
![The Definitive Guide to ARM Cortex-M3 page 123](./the-definitive-guide-to-the-arm-cortex-3-page-123.png)
...
![Use PendSV to control context switch](./use-pendsv-to-control-context-switch.png)
