---
title: 机器学习与 MNIST 手写数字识别入门
slug: machine-learning-and-mnist-handwritten-digit-recognition
date: 2026-06-15
author: Chang Wei (昌维) <changwei1006@gmail.com>
language: zh-Hans-CN
category: [AI]
tag: [Machine-Learning, Deep-Learning, PyTorch, MNIST, CNN, Neural-Network, Python]
cover_image: mnist-handwritten-digits-samples.png
---

> 本文是[《从智慧到人工智能：AI 基本概念与发展史入门》](/articles/getting-started-with-ai-and-machine-learning)的续篇。如果你还不了解 AI 的基本概念、函数视角，以及从图灵测试到 Transformer 的发展脉络，建议先阅读上一篇。
>
...
在[《从智慧到人工智能：AI 基本概念与发展史入门》](/articles/getting-started-with-ai-and-machine-learning)中，我们已经介绍了符号主义（写规则）与感知机（从数据学权重）两条早期路线。本文从更现代的视角切入：**机器学习**如何在不手写全部规则的情况下，让计算机自己从数据里找规律——这也是今天大多数 AI 应用背后的核心思路。
...
![传统编程与机器学习对比：传统编程用数据和规则得到输出，机器学习用数据和答案学习规则](./rule-based-programming-vs-machine-learning.svg)
...
上一篇文章已经从 AI 整体视角建立了 $f(x)=y$ 的框架；这里我们把镜头拉近到**机器学习**——它正是当今实现 AI 最主流的一条路线。把各种华丽包装拆掉，机器学习的本质其实非常简单：**它是在寻找一个函数（Function）**。
...
![监督学习流程图：训练数据+标签→模型→预测新数据](./supervised-learning-workflow.svg)
...
![监督学习与无监督学习对比示意图：有标签数据 vs 无标签数据](./supervised-unsupervised-learning-comparison.png)
...
![机器学习三类问题示意图：回归预测连续值，分类划分已知类别，聚类发现未知分组](./machine-learning-problem-types.svg)
...
![MNIST 手写数字数据集样本：0 到 9的灰度手写数字示例](./mnist-handwritten-digits-samples.png)
...
![像素网格示例：数字局部放大显示像素值矩阵](./mnist-pixel-matrix-grid.svg)
...
![向量和矩阵的数学可视化：向量是一维数字列表，矩阵是二维数字表格](./vector-matrix-visualization.svg)
...
![卷积操作演示：输入图像与卷积核运算产生特征图](./two-dimensional-convolution-operation.svg)
...
![Sobel 算子边缘检测效果：提取图像中的轮廓信息](./mnist-sobel-edge-detection.png)
...
![CNN 最大池化操作示意图：2×2窗口取最大值实现下采样](./cnn-max-pooling-operation.png)
...
![全连接神经网络结构：输入层、隐藏层与输出层](./multilayer-neural-network-architecture.svg)
...
![梯度下降示意图：沿着损失函数的梯度方向逐步优化到最小值](./gradient-descent-optimization-path.png)
...
![Softmax 函数示意图：将神经网络原始输出 logits 转换为总和为 1 的概率分布](./softmax-probability-distribution.svg)
...
1. 理解了机器学习是什么——本质上就是**找一个函数**来映射输入和输出
2. 知道了三种学习方式（监督、无监督、强化）和三类问题（回归、分类、聚类）
3. 理解了图像在计算机中的表示——矩阵与像素
4. 补充了向量、矩阵、线性变换等必要数学基础
5. 深入了解了卷积神经网络——卷积、池化、特征提取、逐层学习
6. 掌握了神经网络的训练机制——前向传播、损失函数、反向传播、梯度下降
7. 认识了各种激活函数——为什么需要非线性
8. **实际写了完整的 PyTorch MNIST 代码并跑通了训练和推理**
9. 理解了推理过程和训练的区别
...
2. Rumelhart, D. E., Hinton, G. E., & Williams, R. J. (1986). **Learning representations by back-propagating errors.** *Nature*, 323(6088), 533-536. —— 反向传播算法的经典论文

## 在线课程

1. **吴恩达《机器学习》专项课程** - Coursera / 网易云课堂
2. **吴恩达《深度学习》专项课程** - Coursera / 网易云课堂
3. **李沐《动手学深度学习》** - d2l.ai（提供中文版）
4. **Stanford CS231n: Convolutional Neural Networks for Visual Recognition** - cs231n.stanford.edu
5. **Stanford CS224n: Natural Language Processing with Deep Learning** - web.stanford.edu/class/cs224n/

## 推荐书籍

1. Goodfellow, I., Bengio, Y., & Courville, A. (2016). **Deep Learning.** MIT Press. —— 俗称"花书"，深度学习理论的经典教材
2. 李航 (2019). **统计学习方法（第2版）.** 清华大学出版社. —— 中文世界最好的统计学习入门书之一
3. Géron, A. (2022). **Hands-On Machine Learning with Scikit-Learn, Keras, and TensorFlow (3rd ed.).** O'Reilly. —— 实战派的选择
4. Zhang, A., Lipton, Z. C., Li, M., & Smola, A. J. (2023). **Dive into Deep Learning.** Cambridge University Press. —— 免费在线版：d2l.ai

## 工具与框架文档

1. **PyTorch 官方文档与教程** - pytorch.org
2. **PyTorch MNIST 示例** - github.com/pytorch/examples/tree/main/mnist
3. **TorchVision 文档** - pytorch.org/vision/stable/index.html

## 社区与平台

1. **Kaggle** - kaggle.com（数据科学竞赛平台，有大量入门级项目和数据集）
2. **Papers with Code** - paperswithcode.com（论文 + 代码 + 评测排行榜）
3. **Hugging Face** - huggingface.co（模型库、数据集、Spaces 应用托管）
4. **GitHub Trending** - github.com/trending/python（关注 Python/AI 方向的热门项目）

## 扩展阅读（进阶方向）

1. **Batch Normalization: Accelerating Deep Network Training by Reducing Internal Covariate Shift** - BatchNorm 论文
2. **Deep Residual Learning for Image Recognition** - ResNet 论文，解决了极深层网络的训练问题
3. **An Image is Worth 16x16 Words: Transformers for Image Recognition at Scale** - ViT，Transformer 进军视觉领域的代表作
4. **Attention Is All You Need (The Annotated Transformer)** - 逐行实现 Transformer 的经典博客：nlp.seas.harvard.edu/annotated-transformer
