...
反之，AI研究者Simon Willison提到：“若你程序里的每一内联容都是大型语言模型写的，但你检查和测试过并完全理解，在我看来这就不算是vibe coding，这只是把大型语言模型当成打字助理。”
...
其实**Vibe Coding 是一种风格，AI 辅助 Coding 是一种工作流**——两者并不互斥。写 Demo，或者产品的MVP（Minimum Viable Product，最小可行产品）时可以 Vibe，但是根长期编程的经验，涉及到：
- 身份认证 / 权限验证
- 支付 / 积分
- 并发、事务、数据库迁移
...
**推荐工作流**：新项目跑 `/init` → 人工删冗余、补 gotchas → 在 PR 里当文档一起 review → 随项目演进持续更新。

## 进阶与 2026 趋势

| 话题 | 要点 |
|------|------|
| Plan → Build 工作流 | 先 `@explore` / Plan Agent 调研，再 Build 执行，减少返工 |
| 上下文经济学 | `/compact`、@ 单文件而非整库、小任务 fork 新 session |
| 安全与隐私 | BYOK 敏感代码；`.env` 不进 prompt；了解 OpenCode 云服务的数据策略 |
| MCP 扩展 | 接 Sentry / Linear / DB，让 Agent 读真实运行时上下文 |
| Skills vs Rules | Skills 是可调用能力包；AGENTS.md / Rules 是持久约束 |
| 多工具组合 | Copilot 补全 + OpenCode Agent 的「双刀流」 |
| 2026 计费趋势 | 从「伪无限订阅」转向 credits/token，团队需设 budget 与告警 |
| AI 代码审查 | 提交前让 Plan Agent review diff，或专门建 review subagent |
| 团队落地 | AGENTS.md 进 Git；PR 模板注明 AI 辅助范围与人工 review 点 |
| 常见踩坑 | Accept All 不跑测试；长 session 幻觉累积；模型版本过旧 |

## 结语

AI 辅助 Coding 不等于不写代码。它的价值在于：把重复劳动（boilerplate、测试骨架、文档、常规 refactor）交给 Agent，人专注于意图、架构与验收。
...
工具会变，模型会换代，但 **Plan → 小步执行 → 验证 → Review** 这条纪律不会过时。Vibe Coding 可以玩，生产代码还是请认真对待。
